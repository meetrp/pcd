...
static char Buildflag = 0;      // TI feature for build
static char *Buildfile = NULL;
...
void ConfigMainWindow::closeEvent(QCloseEvent* e)
{
	...
	QMessageBox mb("qconf", "Save configuration?", QMessageBox::Warning,
        ...
        if (Buildflag) {
            mb.setText("");
            mb.setCaption("qbuild");
            mb.setIcon( QMessageBox::NoIcon );
            mb.setButtonText(QMessageBox::Yes, "&Save/Build");
            mb.setButtonText(QMessageBox::No, "&Exit No Save/Build");
	} else {
	...
	case QMessageBox::Yes:
		conf_write(NULL);
                if (Buildflag && creat(Buildfile, S_IRUSR|S_IWUSR) == -1) {
                    perror("ConfigMainWindow::closeEvent create .qbuild\n");
                    exit(1);
                }
	case QMessageBox::No:
		e->accept();
...
{
	static char str[] = "Welcome to the qconf graphicall configuration tool for Linux.\n\n"
...
void ConfigMainWindow::saveSettings(void)
{
	...
	case fullMode :
        ...
        case menuMode :		// added to remove compile warn
            break;
	}
...
int main(int ac, char** av)
{
	...
	progname = av[0];
        ...
        if (strstr(progname, "qbuild") != NULL) {
            char *tmp;
            Buildflag = 1;
            if ((tmp = getenv("TARGET_HOME")) != NULL)	
                asprintf(&Buildfile, "%s/.qbuild", tmp);
            else
                Buildfile = ".qbuild";
	}
	
	configApp = new QApplication(ac, av);
        name = TI_option_parse(ac, av);

/*	if (ac > 1 && av[1][0] == '-') { replaced by TI option parse
		switch (av[1][1]) {
		case 'h':
		case '?':
			usage();
